...
function selectLatestSnapshotsByMessageOrder(messages, snapshots, limit) {
  if (limit <= 0 || snapshots.length === 0) return [];
  const messageIndex = new Map(messages.map((message, index) => [message.id, index]));
  return snapshots.filter((snapshot) => messageIndex.has(snapshot.messageId)).sort((a, b) => {
...
var inactiveSettingWarnings = /* @__PURE__ */ new Set();
...
async function warnInactiveCompatibilitySettings(userId, settings) {
  const inactive = [];
...
function tryRegisterInterceptor() {
  ...
  spindle.registerInterceptor(async (messages, context) => {
    ...
    let snapshots = [];
    try {
      const chatMessages = await spindle.chat.getMessages(chatId, userId);
